		...
		return 'Rp. '.strrev(implode('.',str_split(strrev(strval($angka)),3)));
            ...
            <div class="table-responsive">
              <table class="table">
                ...
                <tbody>
                  <p hidden> {{ $i = 1 }}</p>
                  @foreach ($data_penyediaanbenih as $key=>$value)
                  <tr>
                    <td>{{ $i++ }}</td>
                    <td>{{ $value-> Bulan}}</td>
                    {{-- <td>{{ convert_to_rupiah( $value-> Target)}}</td> --}}
                    <td>{{ convert_to_rupiah( $value-> Realisasi)}}</td>
                    @if (auth::user()->email == 'admin@gmail.com' || auth::user()->email == 'adminmonev@gmail.com' || auth::user()->email == 'adminkeuangan@gmail.com')
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ url('anggaran_PenyedianBenih') }}" method="POST">
          @csrf
...
<div class="modal fade" id="modal_edittarget" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ url('anggaran_PenyedianBenih/1') }}" method="POST">
          @csrf
...
<div class="modal fade" id="modal_Edit" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ url('anggaran_PenyedianBenih') }}" method="POST" id="editForm">
          @csrf
          {{-- {{method_field('PUT')}} --}}
          <input type="hidden" name="_method" value="PATCH">
          <div class="form-group row mb-3">
...
{{-- MODAL EDIT --}}
<script>
  var data_ikm = {!! json_encode($data_penyediaanbenih -> toArray())!!};
  function updateData(id) {
    ...
    $('#editForm').attr('action', '/anggaran_PenyedianBenih/' + id)
  }
</script>
...
<script>
  ...
  for (let index = 0; index < data_ikm.length; index++) {
    ...
    var Realisasi = parseInt(data_ikm[index].Realisasi);
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", function (event) {
    var Januari = parseInt(data_ikm[0].Realisasi);
    var Febuari = Januari + parseInt(data_ikm[1].Realisasi);
    var Maret = Febuari + parseInt(data_ikm[2].Realisasi);
    var April = Maret + parseInt(data_ikm[3].Realisasi);
    var Mei = April + parseInt(data_ikm[4].Realisasi);
    var Juni = Mei + parseInt(data_ikm[5].Realisasi);
    var Juli = Juni + parseInt(data_ikm[6].Realisasi);
    var Agustus = Juli + parseInt(data_ikm[7].Realisasi);
    var September = Agustus + parseInt(data_ikm[8].Realisasi);
    var Oktober = September + parseInt(data_ikm[9].Realisasi);
    var November = Oktober + parseInt(data_ikm[10].Realisasi);
    var Desember = November + parseInt(data_ikm[11].Realisasi);
